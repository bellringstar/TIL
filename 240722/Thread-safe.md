...
3. Atomic
   ...
   - 원자적 연산을 제공합니다. 이들은 멀티스레드 환경에서 동기화 없이도 안전하게 사용할 수 있는 변수들을 제공합니다.
   - 로우레벨 동작
     - Compare-and-Swap(CAS)연산
       - Atomic class들의 핵심은 CAS 연산입니다.
       - CAS는 메모리 위치의 내용을 주어진 값과 원자적으로 비교하고, 같은 경우에만 새로운 값으로 대체합니다.
       - 기본 로직: CAS(메모리 주소, 예상 값, 새로운 값)
       - 동작 과정
         - 메모리 주소의 현재 값을 읽습니다.
         - 읽은 값이 예상 값과 같은지 비교합니다.
         - 같다면 새로운 값으로 업데이트합니다. 다르다면 업데이트하지 않고 실패를 반환합니다.
     - 하드웨어 지원
       - 대부분의 현대 CPU는 CAS에 해당하는 특별한 명령어를 제공합니다.
       - x86/x65: CMPXCHG 명령어
       - ARM: LDREX와 STREX 명령어 쌍
     - 메모리 배리어
       - 필요한 메모리 배리어를 자동으로 삽입합니다.
       - 이는 연산의 순서와 가시성을 보장합니다.
     - lock-free 알고리즘
       - 내부적으로 락을 사용하지 않습니다.
       - 대신 CAS 기반의 락프리 알고리즘을 사용하여 동시성을 관리합니다.
       - 동작 방식
         - 공유 데이터 구조에 대한 변경을 시도합니다.
         - 변경 시도가 실패하면 (다른 스레드가 먼저 변경한 경우) 재시도합니다.
         - 무한 대기 없이 작업을 완료합니다.
     - 일부 Atomic 연산은 대기 없는 특성을 가집니다.
   - 단점과 한계
     - 메모리 사용량 증가
       - 추가적인 메타데이터와 패딩이 필요하기에 일반 변수보다 더 많은 메모리를 사용합니다.
     - 성능 오버헤드
       - 높은 경합 상황에서는 성능이 저하될 수 있습니다.
       - CAS 연산의 반복적인 실패로 재시도가 발생하기 때문입니다.
       - 경합이 심한 경우 다른 동기화 메커니즘(예:락)을 고려할 수 있습니다.
     - 복합 연산의 한계
       - 여러 Atomic 변수를 포함하는 복합 연산의 원자성을 보장하지 않습니다.
       - 각 Atomic 변수는 개별적으로만 원자성을 제공합니다.
       - 복잡한 원자적 연산이 필요한 경우 synchronized 블록이나 Lock을 사용해야 합니다.
     - ABA 문제
       - 일부 Atomic 클래스(예: AtomicReference)는 ABA 문제에 취약합니다.
       - 값의 중간 변경을 감지하지 못할 수 있습니다.
       - AtomicStampedReference나 AtomicMarkableReference를 사용합니다.
     - 가독성과 유지보수성
4. 불변 객체
   - 객체의 상태를 변경할 수 없게 만들어 Thread-safe를 보장합니다.
   - 동기화가 필요 없어 성능이 좋지만 새 객체 생성으로 인한 오버헤드가 발생할 수 있습니다.
5. ThreadLocal
   - 각 스레드에 고유한 변수를 제공합니다. 이를 통해 여러 스레드가 동시에 실행되더라도 각 스레드는 자신만의 독립적인 변수를 가질 수 있습니다.
   - 스레드 간 간섭을 방지하지만 메모리 사용량이 증가할 수 있습니다.
   - 주요 특징
     - 스레드 독립성: 각 스레드는 자신만의 변수 저장 공간을 갖습니다.
     - 스레드 안전성: 명시적인 동기화 없이도 스레드 안전한 코드를 작성할 수 있습니다.
     - 컨텍스트 저장: 스레드의 컨텍스트 정보를 저장하는 데 유용합니다.
   - 동작 원리
     - 내부 구조
       - ThreadLocal은 내부적으로 Thread 클래스의 ThreadLocalMap을 사용합니다.
       - 각 Thread 객체는 자신만의 ThreadLocalMap을 가집니다.
     - 값 저장 및 검색
       - ThreadLocal.save(value): 현재 스레드의 ThreadLocalMap에 값을 저장합니다.
         - 현재 실행중인 Thread 객체를 얻습니다.
         - Thread 객체의 ThreadLocalMap을 가져옵니다.
         - ThreadLocalMap에 (현재 ThreadLocal 객체, 설정할 값) 쌍을 저장합니다.
       - ThreadLocal.get(): 현재 스레드의 ThreadLocalMap에서 값을 검색합니다.
         - 현재 실행 중인 Thread 객체를 얻습니다.
         - Thread 객체의 ThreadLocalMap을 가져옵니다.
         - ThreadLocalMap에서 현재 ThreadLocal 객체에 해당하는 값을 검색합니다.
         - 값이 없으면 initialValue() 메서드를 호풀하여 초기값을 반환합니다.
     - 메모리 관리
       - ThreadLocalMap은 ThreadLocal 객체를 약한 참조로 키로 사용합니다.
       - 이는 ThreadLocal 객체가 더 이상 참조되지 않을 때 가비지 컬렉션을 가능하게 합니다.
     - ThreadLocalMap의 구조
       - Entry[] table 형태의 배열을 사용합니다.
       - 각 Entry[]는 WeakReference<ThreadLocal<?>>를 키로 사용합니다.
       - 해시 충돌 해결을 위해 개방 주소법을 사용합니다.
     - 약한 참조는 Java의 참조 유형 중 하나로, 가비지 컬렉션의 동작 방식에 영향을 줍니다.
     - 참조 유형
       - 강한 참조: 일반적인 객체 참조
       - 소프트 참조: 메모리가 부족할 때 GC의 대상이 됨
       - 약한 참조: 다음 GC 주기에 수집됨
         - 따라서 캐시 구현이나 객체의 생명주기 관리에 사용됩니다.
       - 팬텀 참조: 객체 삭제되기 전에 정리 작업을 수행하는 데 사용
6. Lock
   - java.util.concurrent.locks 패키지의 Lock 인터페이스를 구현한 클래스들을 사용합니다.
   - synchronized보다 세밀한 제어가 가능하지만 사용법이 복잡할 수 있습니다.
   - 특징
     - 더 나은 성능: 일부 상황에서 synchronized 보다 더 나은 성능을 제공합니다.
     - 유연성: 락 획득 시도의 타임아웃, 락 폴링, 인터럽트 가능한 락 획득 등을 지원합니다.
     - 공정성 옵션: 스레드들 간의 공정한 락 획득 순서를 보장할 수 있습니다.
     - 다양한 조건 지원: 여러 Condition 객체를 사용하여 더 복잡한 동기화 시나리오를 구현할 수 있습니다.
   - 주요 구현체
     - ReentrantLock
       - 가장 일반적으로 사용되는 Lock 구현체입니다.
       - 재진입이 가능합니다.(같은 스레드가 중첩하여 락을 획득할 수 있음)
       - 공정/비공정 락 옵션을 제공합니다.
     - ReadWriteLock
       - 읽기 작업과 쓰기 작업에 대해 서로 다른 락을 제공합니다.
       - 여러 스레드가 동시에 읽기 락을 획득할 수 있어, 읽기 위주의 작업에서 성능 향상을 가져올 수 있습니다.
     - StampedLock
       - Java 8에서 도입된 더 복잡하고 성능이 개선된 락입니다.
       - 낙관적 읽기 락을 제공하여 읽기/쓰기 경합 상황에서 더 나은 성능을 제공할 수 있습니다.
   - Lock의 고급기능
     - tryLock()
       - 락을 즉시 획득하려고 시도하며 성공 여부를 boolean으로 반환합니다.
       - 락을 기다리지 않고 다른 작업을 수행할 수 있게 해줍니다.
     - tryLock(long time, TimeUnit unit)
       - 지정된 시간 동안만 락 획득을 시도합니다.
       - 데드락을 방지하는 데 유용합니다.
     - lockInterruptibly()
       - 락 획득 대기 중인터럽트 될 수 있는 락 획득 방법입니다.
       - 스레드 인터럽트를 이용한 작업 취소에 유용합니다.
     - newCondition()
       - Condition 객체를 생성합니다.
       - 복잡한 동기화 시나리오를 구현할 수 있게 해줍니다.
7. Concurrent
